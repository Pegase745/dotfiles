...
local function system_hibernate ()
   awful.prompt.run {
      prompt       = "Hibernate (type 'y' to confirm)? ",
      ...
      completion_callback = function (t, p, n)
         return awful.completion.generic(t, p, n, {"n", "N", "y", "Y"})
      end
   }
end
...
local function system_hybrid_sleep ()
   awful.prompt.run {
      prompt       = "Hybrid Sleep (type 'y' to confirm)? ",
      textbox      = awful.screen.focused().mypromptbox.widget,
      exe_callback = function (t)
         if string.lower(t) == "y" then
...
local function system_reboot ()
   awful.prompt.run {
      prompt       = "Reboot (type 'y' to confirm)? ",
      textbox      = awful.screen.focused().mypromptbox.widget,
      exe_callback = function (t)
         if string.lower(t) == "y" then
...
local function system_power_off ()
   awful.prompt.run {
      prompt       = "Power Off (type 'y' to confirm)? ",
      textbox      = awful.screen.focused().mypromptbox.widget,
      exe_callback = function (t)
         if string.lower(t) == "y" then
            awesome.emit_signal("exit", nil)
...
awful.rules.rules = {
    ...
    { rule_any = { class = { "Thunar" } },
      properties = { floating = true, width = 1000, height = 670, x = 320, y = 55 }
    },
